...
function readInput() {
  ...
  if(formLength % 2 != 0) {
    ...
    chrome.runtime.sendMessage({length: formLength, sendWords: wordsArr, sendTransl: translationsArr});
  }
...
function beginAlarm() {
  ...
  if(!parseInt(freq)) {
    ...
    chrome.alarms.create('learnAlarm', {delayInMinutes: parseFloat(freq), periodInMinutes: parseFloat(freq)});
